...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(procedure, rest.map(lambda x: scheme_eval(x, env)),
                            env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        tmp = scheme_eval(expressions.first, env, expressions.rest is nil)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first,
                   LambdaProcedure(target.rest, expressions.rest, env))
        return target.first
        # END PROBLEM 9
    else:
...
def do_define_macro(expressions, env):
    ...
    env.define(macro_name, MacroProcedure(target.rest, expressions.rest, env))
    return macro_name
    # END Problem 20
